  ...
  return (
    <section id='contact' className='w-full mt-[15vh]'>
      ...
      <div className="contact-here flex flex-col gap-20 mt-[5vh]">
        <form ref={form} onSubmit={(e) => formHandler(e)} className='w-full flex flex-col gap-8' name='contact'>
          <input ref={email} name='from_email' className='w-full outline-none py-2 bg-transparent border-b-2 border-[#f8f8f8] placeholder:text-[#f8f8f8]' placeholder='Your email' type="email" required/>
          <textarea ref={message} name='message' className='w-full outline-none bg-transparent border-b-2 border-[#f8f8f8] placeholder:text-[#f8f8f8]' placeholder='Message' rows={6} id="connnect-message" required></textarea>
          <button type='submit' value="send" className='text-[#242329] bg-[#27DFB3] font-medium py-2.5 w-[45vw] rounded-full'>Send Message</button>
        </form>
